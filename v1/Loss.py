...
class SAMLoss(torch.nn.Module):
    ...
    def forward(self, y, gt):
        """
        Args:
            y: (B, C, H, W) predicted hyperspectral image
            gt: (B, C, H, W) ground truth hyperspectral image
        Returns:
            sam_loss: mean spectral angle in radians
        """
        # Reshape to (B, H*W, C)
        b, c, h, w = y.shape
        y_v = y.permute(0, 2, 3, 1).reshape(b, h * w, c)
        gt_v = gt.permute(0, 2, 3, 1).reshape(b, h * w, c)

        # Compute dot product and norms
        dot = torch.sum(y_v * gt_v, dim=-1)  # (B, H*W)
        y_norm = torch.linalg.norm(y_v, dim=-1)  # (B, H*W)
        gt_norm = torch.linalg.norm(gt_v, dim=-1)  # (B, H*W)

        # Cosine similarity
        cos = dot / (y_norm * gt_norm + self.eps)
        cos = torch.clamp(cos, -1.0, 1.0)

        # Spectral angle (acos)
        sam = torch.acos(cos)

        return self.weight * torch.mean(sam)


class HybridLossWithSAM(torch.nn.Module):
    """
    Hybrid Loss with SAM component
    结合L1 Loss和SAM Loss，针对SAM优化
    """
    def __init__(self, sam_weight=0.5, l1_weight=1.0, spatial_tv=False, spectral_tv=False):
        super(HybridLossWithSAM, self).__init__()
        self.sam_weight = sam_weight
        self.l1_weight = l1_weight
        self.l1_loss = torch.nn.L1Loss()
        self.sam_loss = SAMLoss(weight=1.0)
        self.use_spatial_TV = spatial_tv
        self.use_spectral_TV = spectral_tv
        self.spatial = TVLoss(weight=1e-3)
        self.spectral = TVLossSpectral(weight=1e-3)
    ...
    def forward(self, y, gt):
        ...
        if self.use_spectral_TV:
            spectral_TV = self.spectral(y)
        total_loss = self.l1_weight * l1 + self.sam_weight * sam + spatial_TV + spectral_TV
        return total_loss
