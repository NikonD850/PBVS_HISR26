...
# 复用 train_sr_x4_h5.py 的导入方式：允许直接运行脚本
# 这里我们依赖 VolFormer 目录下的 tiff_utils（GDAL）提供：
# - write_tiff_chw / float_to_u8
# 并且我们自己在此脚本中做 H5 读取与滑窗推理。
from tiff_utils import write_tiff_chw, float_to_u8
from OverallModel import General_VolFormer
from basicModule import default_conv


def _save_preview_png(out_png: str, cube_chw: np.ndarray, band_indices):
    if imageio is None:
        return
    if cube_chw.ndim != 3:
        return

    c, _h, _w = cube_chw.shape
    ...
    safe_bands = [min(int(b), c - 1) for b in band_indices]

    if len(safe_bands) == 0:
        img = to_u8(cube_chw[0])
    elif len(safe_bands) == 1:
        img = to_u8(cube_chw[safe_bands[0]])
    else:
        while len(safe_bands) < 3:
            safe_bands.append(safe_bands[-1])
        r = to_u8(cube_chw[safe_bands[0]])
        g = to_u8(cube_chw[safe_bands[1]])
        b = to_u8(cube_chw[safe_bands[2]])
...
def main():
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default="/home/fdw/code/HISR/lzy/code/VolFormer/checkpoints/best.pth",
        help="best.pth 权重路径（mains.py 的 save_checkpoint 产物）",
    )
    parser.add_argument(
        "--test_dir",
        type=str,
        default="/home/fdw/data/HISR/final_test",
        help="输入目录（包含 LR_*.h5）",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/home/fdw/code/HISR/lzy/code/VolFormer/result/result_h5",
        help="保存 SR 结果目录（tif/preview/h5）；传空字符串则不保存",
    )
    parser.add_argument(
        "--submit_dir",
        type=str,
        default="/home/fdw/code/HISR/lzy/code/VolFormer/result/submit_h5",
        help="额外复制一份最终h5到该目录下的x4子目录；传空字符串则不复制",
    )
    parser.add_argument(
        "--zip_name",
        type=str,
        default="submit_x4.zip",
        help="最终提交zip文件名（生成在submit_dir同级目录）；传空字符串则不打包",
    )
    parser.add_argument(
        "--preview_dir",
        type=str,
        default="",
        help="保存preview PNG的目录；默认空表示与save_dir一致；传空字符串且save_dir也为空则不保存",
    )
    parser.add_argument(
        "--preview_bands",
        type=str,
        default="10,50,100",
        help="preview用的band索引，例如 '0' 或 '10,50,100'",
    )
    ...
    # 调试参数
    parser.add_argument("--debug_h5", action="store_true", help="开启h5结构调试，处理完第一个文件后打印结构并退出")
    ...
    if not os.path.exists(args.ckpt):
        raise FileNotFoundError(f"ckpt不存在: {args.ckpt}")

    h5_files = sorted(glob.glob(os.path.join(args.test_dir, "LR_*.h5")))
    if not h5_files:
        raise FileNotFoundError(f"在test_dir未找到LR_*.h5: {args.test_dir}")

    net, ckpt_args = _load_ckpt_like_mains(args.ckpt)

    # 兼容：如果 ckpt 里保存的不是模型对象（理论上不该），则按 args 重建
    ...
    preview_dir = args.preview_dir.strip() if isinstance(args.preview_dir, str) else ""
    if preview_dir == "":
        preview_dir = save_dir
    if preview_dir is not None and isinstance(preview_dir, str) and preview_dir.strip() == "":
        preview_dir = None

    preview_bands = [int(x) for x in args.preview_bands.split(",") if x.strip()]
    ...
    if preview_dir is not None and imageio is not None:
    ...
    for h5_path in h5_files:
        base = os.path.splitext(os.path.basename(h5_path))[0]  # LR_00
        sid = base

        infos = _list_h5_datasets(h5_path)
        lr_key = _find_lr_dataset(infos)
        if lr_key is None:
            raise KeyError(f"未在h5中找到LR dataset: {h5_path}, datasets={[n for n,_,_,_ in infos]}")
        ...
        if save_dir is not None:
            out_tif = os.path.join(save_dir, f"{sid}_SR.tif")
            write_tiff_chw(out_tif, sr_chw_u8)

            if imageio is not None:
                out_png = os.path.join(save_dir, f"{sid}_SR_preview.png")
                _save_preview_png(out_png, sr_chw_u8, preview_bands)
            ...
            if submit_dir is not None:
                ...
                shutil.copy2(out_h5, os.path.join(x4_dir, os.path.basename(out_h5)))

        if args.debug_h5 and save_dir is not None:
            print("\n[DEBUG_H5] generated:", out_h5)
            for name, shape, dtype, comp in _list_h5_datasets(out_h5):
                print("  DS", name, "shape", shape, "dtype", dtype, "compression", comp)

            print("\n[DEBUG_H5] exit after first file")
            return
    ...
    if submit_dir is not None:
        ...
        if zip_name is not None:
            ...
            if not os.path.isdir(src_dir):
                raise FileNotFoundError(f"未找到待打包目录: {src_dir}")
    ...
    print(f"[DONE] h5推理完成, total_time={dt_all:.2f}s")
