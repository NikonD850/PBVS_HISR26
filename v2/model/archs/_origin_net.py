...
from ._basic_module import *
import numpy as np
from ._volformer_core import *
...
class General_VolFormer(nn.Module):
    def __init__(
        ...
        if vf_embed_dim % vf_num_heads != 0:
            raise ValueError(f"vf_embed_dim({vf_embed_dim}) must be divisible by vf_num_heads({vf_num_heads})")

        depths = [vf_depth] * vf_layers
        num_heads = [vf_num_heads] * vf_layers
        ...
        self.branch1 = VolFormer(
            ...
            img_range=1,
            depths=depths,
            embed_dim=vf_embed_dim,
            num_heads=num_heads,
            mlp_ratio=2,
            upscale=n_scale,
            use_checkpoint=vf_use_checkpoint,
        )
    ...
    def forward(self, x, lms, modality,img_size):
        ...
        if modality == "spectral":
            ...
            y = torch.zeros(b, c, self.sca * h, self.sca * w, device=x.device, dtype=x.dtype)

            channel_counter = torch.zeros(c, device=x.device, dtype=x.dtype)
            for g in range(self.G):
