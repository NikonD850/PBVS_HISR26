...
class TL(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.mlp = FeedForward(dim,mlp_hidden_dim,bias=False)   
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            TL(dim=dim, input_resolution=input_resolution,
...
class VolFormer(nn.Module):
    ...
    def __init__(self, img_size=[224,224], patch_size=1, in_chans=3,out_chans=1,
                 embed_dim=96, depths=[6, 6, 6, 6], num_heads=[6, 6, 6, 6],
                 window_size=7, mlp_ratio=4., upscale = 4, qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False,img_range=1., upsampler='pixelshuffle', resi_connection='3conv',
                 **kwargs):
        super(VolFormer, self).__init__()
        num_in_ch = in_chans
        ...
        self.conv_after_body3 = nn.Sequential(nn.Conv2d(embed_dim, embed_dim // 4, 3, 1, 1),
                                                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                nn.Conv2d(embed_dim // 4, embed_dim // 4, 1, 1, 0),
                                                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                nn.Conv2d(embed_dim // 4, embed_dim, 3, 1, 1))
    ...
    def forward_features1(self, x,img_size):
    ...
    def forward(self, x,img_size):
        ...
        res1 = self.conv_after_body1(self.forward_features1(x_first,img_size)) + x_first
        
        x=self.conv_before_upsample(res1)
